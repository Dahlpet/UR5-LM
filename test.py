import Leap, sys, URBasic, URBasic.urScript, time, socket, URleap, threading
    ...
    def on_frame(self, controller):
        ...
        for hand in frame.hands:
            ...
            if (time.time() - self.last_print_time) > self.print_interval:
                ...
                # Define a function for each move command
                def move_to_point(x, y, z):
                    robot.movel(pose=[x, y, z, 0, -3.14, 0], a=acc, v=vel)

                def turn_wrist(rx, ry, rz):
                    robot.movej(pose=[0, 0, 0, rz, rx-3.14, ry], a=acc, v=vel)

                # Create a new thread for each move command and start them simultaneously
                point_thread = threading.Thread(target=move_to_point, args=(new_x, new_y, new_z))
                wrist_thread = threading.Thread(target=turn_wrist, args=(rx, ry, rz))
                point_thread.start()
                wrist_thread.start()
